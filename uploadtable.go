...
func writeUploadRow(b *strings.Builder, r UploadRow) {
	...
	// input through the HTML form owner attribute. A failed booking is flagged
	// right here -- there is no status column to carry it.
	b.WriteString(`<td class="num"><input class="g" form="` + formID + `" type="number" name="g" min="0" step="1" value="` +
	...
	b.WriteString(`<td class="act"><form id="` + formID + `" method="POST" action="/book">` +
		...
		`<button name="action" value="book" title="set to the G-code amount">Book</button>` +
		`<button name="action" value="set" title="book the amount entered">Set</button>` +
		`<button name="action" value="cancel" class="danger" title="refund back to 0 g">Cancel</button>` +
		`</form></td>`)
	b.WriteString(`</tr>`)
}
